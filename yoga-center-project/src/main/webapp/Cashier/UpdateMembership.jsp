<%-- 
    Document   : UpdateMembership
    Created on : Jul 19, 2023, 1:49:44 PM
    Author     : localboss
--%>
                            ...
                            <div class="input__group filter">
                                 <select class="input-filter" name="txtStatus" >
                                    <c:forEach items="${StatusList}" var="item">
                                        <option ${MEMBERSHIP.status==item.ordinal()?'selected':''} value="${item.ordinal()}" >${item.name()}</option>
                                    </c:forEach>
                                </select>
